...
def decode_frames(bitstream, quantization_step):
    ...
    first_frame = 0.0
    reconstructed_frames = np.zeros(num_frames+1)
    reconstructed_frames[0] = first_frame
    previous_reconstructed_frame = first_frame
    for i in range(1, num_frames+1):
        # 将量化残差反量化
        decoded_residual = decoded_quantized_residuals[i-1]
...
def decode_to_csv(header, file_folder, input_folder, output_folder, start_time):
